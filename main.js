...
async function unlockAudio() {
  ...
  if (!window.__fwAudioContext) window.__fwAudioContext = new AudioContext();
  if (window.__fwAudioContext.state === 'suspended') await window.__fwAudioContext.resume();
  audioUnlocked = true;
}
...
function loadSounds() {
  ...
  Object.values(sounds).forEach(a => { a.preload = 'auto'; a.volume = 0.75; });
}
...
function onHandResults(results) {
  if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
    ...
    if (openness > 0.21) {
      ...
      statusEl.textContent = showFireworks ? '手のひら検出中：花火表示' : '手のひら検出中：花火開始を押してください';
      return;
    }
  }
  handVisible = false;
...
function getPalmCenter(lm) {
  ...
  const p = ids.reduce((acc, id) => ({ x: acc.x + lm[id].x, y: acc.y + lm[id].y }), { x: 0, y: 0 });
...
function spawnLaunch() {
  const origin = palmWorld.clone().add(new THREE.Vector3(0, -0.55, 0));
  const target = palmWorld.clone().add(new THREE.Vector3(0, 0.55, 0));
  for (let i = 0; i < 90; i++) {
    const t = i / 90;
    const p = origin.clone().lerp(target, t);
    p.x += rand(-0.025, 0.025);
    p.y += rand(-0.025, 0.025);
    createSpark(p, new THREE.Vector3(rand(-0.05,0.05), rand(0.55,1.2), rand(-0.04,0.04)), 0xffd36b, 0.9, rand(0.018,0.034), 'launch');
  }
  playSound('launch');
}

function spawnExplosion() {
  const center = palmWorld.clone().add(new THREE.Vector3(0, 0.65, 0));
  for (let i = 0; i < 620; i++) {
    const dir = randomSphereDirection();
    const speed = rand(0.8, 2.7);
    const color = CONFIG.colors[Math.floor(Math.random() * CONFIG.colors.length)];
    createSpark(center, dir.multiplyScalar(speed), color, rand(1.3, 2.2), rand(0.018, 0.045), 'explode');
  }
  for (let i = 0; i < 65; i++) createSmoke(center.clone().add(randomSphereDirection().multiplyScalar(rand(0.05, 0.55))));
  playSound('explosion');
}

function spawnTextGather() {
  if (textTargets.length === 0) return;
  const center = palmWorld.clone().add(new THREE.Vector3(0, 0.58, 0));
  const count = Math.min(CONFIG.maxSparks, textTargets.length);
  for (let i = 0; i < count; i++) {
    const target2 = textTargets[i % textTargets.length];
    const target3 = center.clone().add(new THREE.Vector3(target2.x, target2.y, rand(-0.05, 0.05)));
    const start = center.clone().add(randomSphereDirection().multiplyScalar(rand(0.7, 1.35)));
    const color = CONFIG.colors[i % CONFIG.colors.length];
    createSpark(start, randomSphereDirection().multiplyScalar(rand(0.1, 0.45)), color, rand(2.3, 3.4), rand(0.016, 0.035), 'text', target3);
  }
  playSound('sparkle');
}
...
function createSpark(position, velocity, colorHex, life, size, mode, target = null) {
  ...
  sparks.push({
    p: position.clone(), v: velocity.clone(), color, life, maxLife: life, size,
    mode, target, age: 0, wobble: rand(1, 4), phase: rand(0, Math.PI * 2), alpha: 1,
  });
}

function createSmoke(position) {
  if (smokes.length >= CONFIG.maxSmoke) smokes.shift();
  smokes.push({
    p: position.clone(), v: new THREE.Vector3(rand(-0.08,0.08), rand(0.03,0.16), rand(-0.04,0.04)),
    life: rand(1.2, 2.2), maxLife: 2.2, size: rand(0.10, 0.22), alpha: rand(0.16, 0.33),
  });
}
...
function animate() {
  ...
  const visible = showFireworks && handVisible && (now - lastHandTime < CONFIG.handLostFadeSec);
  if (!visible) {
  ...
  if (visible && fireworkRunning) {
    launchTimer += dt;
    if (launchTimer > 0.75 && launchTimer < 0.75 + dt) spawnExplosion();
    if (launchTimer > 1.55 && launchTimer < 1.55 + dt) spawnTextGather();
    if (launchTimer > 4.0) resetFirework(true);
  }
...
function updateSparks(dt, visible) {
  ...
  for (let i = 0; i < CONFIG.maxSparks; i++) {
    ...
    const j = i * 3;
    if (s) {
      sparkPositions[j] = s.p.x; sparkPositions[j+1] = s.p.y; sparkPositions[j+2] = s.p.z;
      sparkColors[j] = s.color.r; sparkColors[j+1] = s.color.g; sparkColors[j+2] = s.color.b;
      sparkSizes[i] = s.size * (s.mode === 'text' ? 1.15 : 1.0);
      sparkAlphas[i] = s.alpha;
    } else {
      sparkPositions[j] = sparkPositions[j+1] = sparkPositions[j+2] = 999;
      sparkSizes[i] = sparkAlphas[i] = 0;
    }
  }
...
function updateSmokes(dt, visible) {
  ...
  for (let i = 0; i < CONFIG.maxSmoke; i++) {
    ...
    const j = i * 3;
    if (s) {
      smokePositions[j] = s.p.x; smokePositions[j+1] = s.p.y; smokePositions[j+2] = s.p.z;
      smokeSizes[i] = s.size;
      smokeAlphas[i] = s.alpha * Math.max(0, s.life / s.maxLife);
    } else {
      smokePositions[j] = smokePositions[j+1] = smokePositions[j+2] = 999;
      smokeSizes[i] = smokeAlphas[i] = 0;
    }
  }
